...
def Find_key_info_all_aa(aa_list_,Folder_path,*args):  # Finds different parameters for each aa (i.e Total_keys,Distinct_keys etc)
    Distict_key_set_ = []
    All_key_dict_ = []
    ...
    for aa in aa_list_:
        ...
        Key_set = set({})
        Key_dict = {}
        Path=f'{Folder_path}/key_{aa}.keys_theta29_dist17'
        if os.path.exists(Path):
            InputKeyFile = open(f'{Folder_path}/key_{aa}.keys_theta29_dist17', 'r')  # Enter the path of the Key file
...
if __name__=="__main__":
    Amino_acid_list = ['TYR'] # Change the number of aa here
    for data in Amino_acid_list:
        ...
        Distict_key_set, All_key_dict, aa_dis_tot_keys_dict = Find_key_info_all_aa(aa_list,Output_Folder_Path)  # Finds all key information for a particular aa
